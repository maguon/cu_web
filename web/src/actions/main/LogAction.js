...
export const getLogList = () => async (dispatch, getState) => {
    try {
        // 检索条件：开始位置
        ...
        if (res.success === true) {
            dispatch({type: LogActionType.setDataSize, payload: res.result.length});
            dispatch({type: LogActionType.getLogList, payload: res.result.slice(0, size - 1)});
        } else if (res.success === false) {
            swal('获取发货列表信息失败', res.msg, 'warning');
        }
    } catch (err) {
...
export const getLogCoList = () => async (dispatch) => {
    try {
        ...
        const res = await httpUtil.httpGet(url);
        if (res.success === true) {
            dispatch({type: LogActionType.getLogCoList, payload: res.result});
        } else if (res.success === false) {
