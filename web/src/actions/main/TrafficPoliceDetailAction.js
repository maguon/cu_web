...
export const getPoliceInfo = (id) => async (dispatch, getState) => {
    try {
        // 基本检索URL
        const url = apiHost + '/api/admin/' + localUtil.getSessionItem(sysConst.USER_ID) + '/supervise?superviseId=' + id;
...
export const changeStatus = (id) => async (dispatch, getState) => {
    swal({
        ...
        if (isConfirm && isConfirm.value === true) {
            ...
            const url = apiHost + '/api/admin/' + localUtil.getSessionItem(sysConst.USER_ID) + '/supervise/' + id + '/status/' + status;
...
export const updatePolice = (id) => async (dispatch, getState) => {
    ...
    try {
        if (name === '' || phone === '') {
            ...
            const url = apiHost + '/api/admin/' + localUtil.getSessionItem(sysConst.USER_ID) + '/supervise/' + id;
