...
export const getOrderInfo = (id) => async (dispatch) => {
    try {
        ...
        if (res.success === true) {
            dispatch({type: OrderDetailActionType.getUserCarInfo, payload: res.result});
...
export const getCheckCarList = (id) => async (dispatch, getState) => {
    try {
        ...
        const res = await httpUtil.httpGet(url);
        if (res.success === true) {
            dispatch({type: OrderDetailActionType.setDataSize, payload: res.result.length});
            dispatch({type: OrderDetailActionType.getCheckCarList, payload: res.result.slice(0, size - 1)});
        } else if (res.success === false) {
